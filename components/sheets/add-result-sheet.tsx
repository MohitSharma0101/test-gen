...
export const AddResultSheet = ({ open, onOpenChange, batch, date, defaultExamResult, onSubmit }: Props) => {
  const { users, loading: usersLoading } = useUsers({
    batchId: batch._id,
    shouldLoad: !!batch._id,
  });

  const [totalMarks, setTotalMarks] = useState(defaultExamResult?.totalMarks ?? 100);
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side={"right"} className="p-0 flex flex-col gap-2 max-h-screen">
        ...
        <Button disabled={viewMode || isSubmitting || isFormInvalid} className="mb-2 mx-2" onClick={handleSubmit}>
          Submit
        </Button>
      </SheetContent>
    </Sheet>
