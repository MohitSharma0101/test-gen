...
import { apiErrorMsg, getScheduleStatus, withConfirmation } from "@/lib/utils";
...
const SchedulePaperPage = (props: Props) => {
  ...
  const { schedulePapersRes, loading, error, refresh } = useSchedulePapers({
  ...
  const onDelete = (schedulePaperId: string) => {
    withConfirmation(async () => {
      try {
        await api.delete(ENDPOINT.schedulePaper, {
          params: {
            schedulePaperId: schedulePaperId,
          },
        });
        toast.success("Schedule paper deleted successfully!");
        refresh();
      } catch (err) {
        toast.error(apiErrorMsg(err, "Unable to delete schedule paper!"));
      }
    });
  };
        ...
        {schedulePapersRes?.schedulePapers.map((item) => (
          <div key={item._id} className="border rounded-md p-2 md:px-3 bg-white">
            <div className="text-slate-950 font-semibold flex items-center gap-2">
              ...
              {isLive(item) && (
                <div className="min-w-fit max-h-fit rounded-full text-xs px-2 py-1 flex items-center justify-center bg-blue-400 text-white">
                  Live
                </div>
              )}
              <DotsMenu
                ...
                config={[
                  ...
                  },
                  {
                    ...
                    label: "Delete",
                    className: "text-white bg-red-400 focus:bg-red-500 focus:text-white",
            ...
            <div className="py-2 grid grid-cols-2 md:grid-cols-3 gap-1 md:gap-2">
              <LabelValue label="Duration" value={`${item.duration} min`} />
              <LabelValue label={"Starts At"} value={Clock.getDateTimeInFormat(item.startTime)} />
              <LabelValue label={"Expires At"} value={Clock.getDateTimeInFormat(item.endTime)} />
            </div>
          </div>
        ))}
        {viewPaperId && (
